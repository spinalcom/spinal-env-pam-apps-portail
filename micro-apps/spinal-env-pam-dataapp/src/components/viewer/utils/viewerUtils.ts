...
import { getModelById, getModelList } from '../ModelDictionnary';
import { IDbIdModelAggregate, SceneAlignMethod, IStartLoadModel, } from 'spinal-viewer-event-manager';

export function initViewer(viewerDiv: HTMLElement): Promise<Autodesk.Viewing.Viewer3D> {
...
export async function load3DModels(viewer: Autodesk.Viewing.Viewer3D, data: IStartLoadModel): Promise<void[]> {
  ...
  return load1stThenAll( tasks, async (d: IloadModelTask): Promise<void> => {
      // const model =
      await loadBimFile(viewer, d.path, data.loadingType, d.id, d.dbids, d.aecPath, data.buildingId);
...
async function loadBimFile(viewer: Autodesk.Viewing.Viewer3D, urlpath: string, sceneAlignMethod: SceneAlignMethod, modelId: string, dbids?: number[],aecPath?: string, buildingId?: string ): Promise<Autodesk.Viewing.Model> {
...
export function viewerSelect(viewer: Autodesk.Viewing.Viewer3D, data: IDbIdModelAggregate[]): void {
  clearSelect(viewer);
  if (data.length === 1) {
    const d = data[0];
