   ...
   <v-card elevation="4" class="cardContainer">
      <div class="dataContainer" v-if="pageSate === PAGE_STATES.loaded && !isBuildingSelected">
         ...
         <div class="detail_container">
            <GroupDataView v-for="(d,i) in data" :key="i" v-if="viewGroup(d)" :data="d" :config="config" :calculMode="calculMode" :unit="unit" />
         </div>
      </div>
      ...
      <div class="centered" v-else-if="pageSate === PAGE_STATES.error">
         ...
         <div color="red">Something went wrong ! Please  
            <v-btn small outlined color="red" @click="retriveData"> retry </v-btn>
         </div>
      </div>
   </v-card>
</template>
...
class InsightApp extends Vue {
   ...
   retriveData() {
      ...
      return this.$store.dispatch(ActionTypes.GET_GROUPS_ITEMS, { config: this.config, buildingId, forceUpdate: true })
         .then((result) => {
   ...
   watchSelectedZone() {
      ...
      this.$store.dispatch(ActionTypes.GET_GROUPS_ITEMS_BY_GEOITEM, playload).then((items) => {
         this.data = calculItemsValue(items, this.calculMode);
      })
   }
