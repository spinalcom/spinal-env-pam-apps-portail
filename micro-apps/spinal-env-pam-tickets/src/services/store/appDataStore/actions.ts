...
import { loadTickets, filterTicketsOnPosition } from "../../store/appDataStore/utils/ticketUtils";
...
export const actions = {
	...
	async [ActionTypes.LOAD_TICKETS]({ commit, dispatch, state }: AugmentedActionContextAppData, { buildingId, config }): Promise<{ [key: string]: INodeItem }> {
		const spinalAPI = SpinalAPI.getInstance();
		...
		if (typeof ApiIteratorStore[ActionTypes.LOAD_TICKETS][buildingId] === "undefined") {
			ApiIteratorStore[ActionTypes.LOAD_TICKETS][buildingId] = spinalAPI.createIteratorCall(loadTickets, buildingId, config);
		}

		const items = await ApiIteratorStore[ActionTypes.LOAD_TICKETS][buildingId].next();
		return items?.value;
	},

	async [ActionTypes.FILTER_TICKETS]({ commit, dispatch, state }: AugmentedActionContextAppData, { buildingId, dynamicId }) {
		let ticketsToFilter = await dispatch(ActionTypes.LOAD_TICKETS, {buildingId});
