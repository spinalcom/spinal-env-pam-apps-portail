...
export class ViewerManager {
  ...
  public async loadInViewer(
    item: IPlayload,
    loadOnlyThisModel: boolean = true,
    body?: IViewInfoBody & {
      dbIdsToAdd?: { bimFileId: string; dbIds: number[] }[];
    }
  ) {
  ...
  public async getViewerInfo(
    ...
    if (nodeTofetech.length > 0) {
      ...
      for (const _item of datas) {
        this._viewerStores["GET_VIEWER_INFO"][_item.dynamicId] =
          generator(_item);
        res.push(_item);
      }
    }
  ...
  public async addComponentAsSprites(
    ...
    const emitter = EmitterViewerHandler.getInstance();
    emitter.emit(
      <any>VIEWER_EVENTS.VIEWER_ADD_COMPONENT_SPRITE,
      formatted as any
    );
  }
  ...
  private async _fctViewerIteract(
    ...
    playload: (IPlayload | string) | (IPlayload | string)[]
  ): Promise<any> {
    const emitter = EmitterViewerHandler.getInstance();
