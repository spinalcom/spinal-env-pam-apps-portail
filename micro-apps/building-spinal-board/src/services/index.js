...
export async function getDataBuilding(timestamp, period, buildings, cp, cp_batiment, id_batiment, color) {
  ...
  console.log('le bat', surface);
  let dynamicId = surface.data.dynamicId;
  // console.log('la iss surface',surface.data);
...
export async function getData(timestamp, period, buildings, cp, cp_batiment, id_batiment, color) {
  ...
  for (const floor of floors.data) {
    ...
    // console.log('cllist ', cpList);
    // console.log(cpList.data[0].endpoints);
    ...
    if (endpoint != undefined) {
      ...
      let datatimeSerie = timeSeries1.data
      ...
      if (period === 'Mois') {
        label1.forEach(day => {
          processedTimeSeries1.push(
            datatimeSerie.filter(elem => moment(elem.date).format('DD MMM') === day)
              .reduce((sum, elem) => sum + elem.value, 0)
          );
        });
      } else if (period === 'Année') {
        label1.forEach(month => {
          processedTimeSeries1.push(
            datatimeSerie.filter(elem => moment(elem.date).format('MMM') === month)
              .reduce((sum, elem) => sum + elem.value, 0)
          );
        });
      } else if (period === 'Décennie') {
        label1.forEach(year => {
          processedTimeSeries1.push(
            datatimeSerie.filter(elem => moment(elem.date).format('YYYY') === year)
              .reduce((sum, elem) => sum + elem.value, 0)
          );
        });
      } else if (period === '3 mois') {
        ...
        processedTimeSeries1 = datatimeSerie;
      }
