<template>
  <v-card class="line-card pa-1 rounded-lg d-flex flex-column flex-grow-1" elevation="5" outlined
    ...
    <v-progress-circular style="position: absolute;left: 52.5%; top: 77px;" @click="toto()"
    ...
    <div class="d-flex flex-column flex-grow-1 flex-shrink-1" style="height:0">
      <LineChart :key="chartKey" :data="lineChartData" :chart-id="'99'" :options="lineChartOptions" class="bar-height" />
    </div>
  </v-card>
</template>
    ...
    lineChartData() {
      ...
      for (let i = 0; i < this.datasets.length; i++) {
        if (i == 0) {
          ...
          // console.log('les issoufleur', tempDatasets[i]);
        ...
        interaction: {
          ...
          callbacks: {
            title: (tooltipItem, data) => {
              let label = `Le ${tooltipItem[0].label}`
