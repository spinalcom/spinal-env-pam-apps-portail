...
export class SpriteManager {
  ...
  public async addCardComponent(
    ...
    for (const d of data) {
      ...
      const exists = this.cards3Ds.some(
  ...
  public async addComponentAsSprite(
    ...
    console.warn("data to draw", data);

    data = Array.isArray(data) ? data : [data];
      const dataMapColor: Map<number, string> = new Map();
    const dataMap = new Map();
    data.forEach((d) => dataMap.set(d.dynamicId, d));
    // await this.removeAllLines(viewer);
    data.forEach(el => {
      const color = el.data.data.color;
      if(color) {
        const dynamicId = el.data.data.dynamicId;
        dataMapColor.set(dynamicId, color);
      }
  ...
  public async drawLineBetweenPositions(
    ...
    const color = colorMap.get(source);
    ...
    // console.warn("color of line: ", color);
  ...
  public async selectSprites(dynamicIds: Array<number>) {
    ...
    for (let i = 0; i < dynamicIds.length; i++) {
      ...
      for (let label of this.label3Ds) {
        ...
        for (let line of this.linesGlobal) {
          if (
            label.dynamicId == line.destination &&
            line.source == dynamicIds[i]
          ) {
            // label.component._isSelected();
          }
        }
      ...
      for (let line of this.linesGlobal) {
        if (dynamicIds[i] == line.source) {
          ...
          const color = store.state.appDataStore.hardwareequipementColor.get(line.source);
           if(color) {
          this.updateLine(line.line, new THREE.Color(color), 4); // unselected line
           }
           else {
  ...
  public removeSprites() {
    ...
    this.label3Ds = [];
    // console.log("Sprites removed", this.label3Ds);
  }
