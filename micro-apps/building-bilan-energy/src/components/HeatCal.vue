<template>
    <v-card class="bar-card  d-flex flex-shrink-1 flex-column">
        <v-card-title class="card-title flex-shrink-1 justify-space-between">
            ...
            <div class="d-flex align-center mln6" style="position: absolute; right: calc(50% - 50px); font-size: 14px;"
                v-if="true">
                <v-icon style="margin-top: -1px; font-size: 18px; margin-right: 10px; color: #63717f;">{{
                    env.unit.left.icon }}</v-icon>
                ...
                <v-icon style="margin-top: -1px; font-size: 18px; margin-left: -10px; color: #63717f;">{{
                    env.unit.right.icon }}</v-icon>
            </div>
            <div v-if="true" style="height: 40px; align-self: flex-start; padding-top: 10px; padding-right: 10px;">
                <v-btn :disabled="false" @click="$emit('nav', -1)"
                    style="font-size: 14px !important; border-radius: 10px;  min-width: 36px !important; box-shadow: none; border: 1px solid #EAEEF0 !important;"><v-icon
                        style="color: #14202c !important" icon>mdi-chevron-left</v-icon>{{ prev }}</v-btn>
                <v-btn :disabled="false" @click="$emit('nav', +1)"
                    style="font-size: 14px !important; border-radius: 10px;  min-width: 36px !important; box-shadow: none; border: 1px solid #EAEEF0 !important;">{{
        ...
        <div class="d-flex flex-column flex-grow-1">
            <div class="selects">
                <v-select v-model="space" append-icon="mdi-chevron-down" :items="source" outlined
                    ...
                    style="margin-left: 10px !important; min-width: 100px; width: 300px; flex-grow: 0; font-size: 14px !important;"
                    class="ml-8" :label="selectTitle">
                    <template #label="slotProps">
                        <label v-if="slotProps && slotProps.attrs" :for="slotProps.attrs.id"
                            style="font-size: 14px;">Select an item</label>
                    </template>
                    <template #item="{ item }">
                ...
                <v-select v-model="calculation" append-icon="mdi-chevron-down" :items="calculationList" outlined
                    menu-props="{ bottom: true }" color="#000000DE" item-color="#000000DE" dense
                    style="margin-left: 10px !important; min-width: 100px; width: 130px; flex-grow: 0; font-size: 14px !important;"
                ...
                <div class="selects inside">
                    <v-select v-model="timeRange" append-icon="mdi-chevron-down" :items="timeRanges" outlined
                        menu-props="{ bottom: true }" color="#000000DE" item-color="#000000DE" dense
                        style="margin-left: 10px !important; min-width: 200px; width: 200px; flex-grow: 0; font-size: 14px !important;"
            ...
            <div class="flex-grow-1" v-if="chart === 'calendar'">
                <CalendarAndStripe @dayFilter="dayFilter" @toggle="toggle" @chart-sent="handleChart" :calc="calculation"
                    :results="data" :max="max" :unit="unit" v-if="data && data.d && data.d.length > 0" />
            </div>

            <div class="d-flex flex-row flex-grow-1" style="padding: 10px; width: 100%; justify-content: space-between;"
                v-else-if="chart === 'line'">
                <LineChart :title="'title'" :subtitle="'subtitle'" :labels="chartx.label" :datasets="chartx.data"
                    :next="'temporality.next'" :prev="'temporality.prev'" :stacked="false"
                    style="max-height: 530px; width: 70%" class="BR" />
                <CalendarAndStripeTri @dayFilter="dayFilter" @toggle="toggle" @chart-sent="handleChart"
                    :calc="calculation" :results="data" :max="max" :unit="unit" v-if="data && data.d && data.d.length > 0"
                    style="width: 30%" />
            </div>
        </div>
    </v-card>
