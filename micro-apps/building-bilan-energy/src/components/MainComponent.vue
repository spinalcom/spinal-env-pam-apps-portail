...
class App extends Vue {
  ...
  calculus(calc) {
    ...
    // if (calc === 'Maximum') {
    //   this.stat.text = 'Consommation maximum';
    //   let maxOverall = 0;
    ...
    // if (calc === 'Minimum') {
    //   this.stat.text = 'Consommation minimum';
    ...
    if (calc === 'Moyenne' || true) {
      this.stat.text = 'Consommation moyenne';
      let sumOverall = 0;
      let countOverall = 0;
      ...
      if (this.stat.value === -Infinity || this.stat.value === Infinity || isNaN(this.stat.value)) this.stat.value = 0;
      this.stat.maxCapacity = (this.stat.value * capacity);
    ...
    if (calc === 'Somme') {
      this.stat.text = 'Consommation totale';
      let sumOverall = 0;
      let countOverall = 0;
      let totalValue = this.calendar.rawInterval.reduce((sum, obj) => sum + obj.value, 0);
      this.stat.value = totalValue;
      if (this.stat.value === -Infinity || this.stat.value === Infinity || isNaN(this.stat.value)) this.stat.value = 0;
      this.stat.maxCapacity = (this.stat.value * capacity);
