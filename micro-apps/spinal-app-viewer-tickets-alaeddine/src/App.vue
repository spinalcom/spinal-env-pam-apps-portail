...
<template>
  <v-app v-if="pageSate === PAGE_STATES.loaded" class="app">
    ...
    <div class="dataBody">
      ...
      <dataSideApp class="appContainer" :DActive="isActive3D" :ActiveData="isActive" :config="config" :baseURL="baseUrl"
        :token="token" :selectedZone="selectedZone" :data="displayedData" @changeRoute="changeApp"
    ...
    switch (item?.type) {
      ...
      case "building":
        return (
          ...
          .map((floor) => {
            // Debugging log for floors

            const ticket = data.filter((t) => {
              if (!t.elementSelected?.position?.floor) {
                console.warn("Skipping ticket due to missing position or floor:", t);
                return false;
              }
  ...
  onActionClick({ button, item }) {
    ...
    switch (button.onclickEvent) {
      case ActionTypes.OPEN_VIEWER:
        ...
        // console.log("open viewer case 1");
        break;
      case ActionTypes.ISOLATE_ITEMS:
        ...
        break;
      // console.log("open viewer case 2");
      case "OPEN_VIEWER_PLUS":
        ...
        // console.log("open viewer case 3");
        break;
      default:
        this.$store.dispatch(button.onclickEvent, data);
        // console.log("open viewer case 4");
        break;
    }
  }
