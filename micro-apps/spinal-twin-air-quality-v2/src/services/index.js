...
export async function getFloors(cp) {
  ...
  let promises = result.data.map(async (floor) => {
    ...
    let currentFloorRoomList = await HTTP.get(`floor/${floor.dynamicId}/room_list`);
...
export async function getBuildingEndpoints() {
  ...
  try {
    ...
    const buildingId = localStorage.getItem('idBuilding');
    const floorListResponse = HTTP.get(`floor/list`);
    ...
    const roomListPromises = floorList.data.map(async floor => {
      ...
      const roomListResponse = await HTTP.get(`floor/${floor.dynamicId}/room_list`);
      const roomList = roomListResponse.data;
      const roomPromises = roomList.map(async room => {
        const endpointResponse = await HTTP.get(`node/${room.dynamicId}/control_endpoint_list`);
