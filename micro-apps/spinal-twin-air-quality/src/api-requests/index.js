...
export async function getBuildingAsync() {
  ...
  const result = await HTTP.get(`/building/${buildingId}/geographicContext/space`);
 ...
 */
 export async function getBuildingAsyncV2() {
  ...
  let promise = building.children.map(async currentFloor => {
    ...
    const result = await HTTP.get(`/building/${buildingId}/floor/${currentFloor.dynamicId}/room_list`);
    body = result.data;
    body.map(currentRoom => {
      var foundIndex = building.children.findIndex(x => x.dynamicId==currentFloor.dynamicId);
...
export async function getSoloCpAsync(id, cp) {
  ...
  const result = await HTTP.get(`/building/${buildingId}/node/${id}/control_endpoint_list`);
  const body = result.data;
  var res;
  if(body.length>0)
  res = body[0].endpoints.find(t => {return t.name==cp});
  return res
}
...
export async function getControlEndpointsByNameAsync(cp, building) {
  let cpTable = {name: cp, data: [], equiped: 0, maintainance: 0, noSensor: 0};
  if(cp && building) {
    await getSoloCpAsync(building.dynamicId, cp)
    .then((v) => {
      if(v){
        cpTable.data.push({name: building.name, dynamicId: building.dynamicId, currentValue: v.currentValue, floor: '-'});
      }
    });

    for (let i = 0; i < building.children.length; i++){
      await getSoloCpAsync(building.children[i].dynamicId, cp).then(async(v) => {
        if(v){
          cpTable.data.push({name: '-', dynamicId: building.children[i].dynamicId, currentValue: v.currentValue, floor: building.children[i].name});
          for(let j = 0; j< building.children[i].children.length; j++) {
            await getSoloCpAsync(building.children[i].children[j].dynamicId, cp).then((v1) => {
              if(v1 && v1.currentValue == 0)
                cpTable.maintainance +=1;
              else if(v1){
                cpTable.equiped += 1;
              cpTable.data.push({name: building.children[i].children[j].name, dynamicId: building.children[i].children[j].dynamicId, currentValue: v1.currentValue, floor: building.children[i].name});
              }
 ...
 */
 export async function getContextTreeSpeed(context, path) {
  ...
  const commissioningDynamicId = result.data.find(t => {return t.name==context}).dynamicId;
  result = await HTTP.get(`/building/${buildingId}/groupContext/${commissioningDynamicId}/tree`);
  // TODO 
  // Make it recursive
  result = result.data.children.find(t => {return t.name=='Analyse'}).children.find(t => {return t.name=='Multicapteurs'}).children;
  result = result.map(element => {return {dynamicId: element.dynamicId, name: element.name}})
  let promises = [];
  ...
  for (let index = 0; index<result.length; index++){
    ...
    promises.push(HTTP.get(`/building/${buildingId}/node/${result[index].dynamicId}/control_endpoint_list`))
  }
    // a.find(((element, i) => {
...
export async function getContextTree(context, path) {
  ...
  for (let index = 0; index<result.length; index++){
    a = await (await HTTP.get(`/building/${buildingId}/node/${result[index].dynamicId}/control_endpoint_list`)).data[0].endpoints;
    a.find(((element) => {
      if(element.name=='Convention de nommage'){
        // console.log(a[i].currentValue, 'VS', element.currentValue);
        result[index]['naming'] = element.currentValue;
    ...
    a.find((element => {if(element.name=='Taux de données en défaut reçues'){result[index]['default'] =  element.currentValue}}));
    a.find((element => {if(element.name=='Taux de disponibilité'){
