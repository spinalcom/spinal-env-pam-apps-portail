...
export async function getRoomPositions(buildingId: string, dynIds: string[]): Promise<IRoomPositionRes[]> {
  const spinalAPI = SpinalAPI.getInstance();
  const url = spinalAPI.createUrlWithPlatformId(buildingId, '/api/v1/room/get_position_multiple');
  try {
      const response = await spinalAPI.post<IRoomPositionRes[]>(url, dynIds); // Envoyer le tableau d'identifiants
      return response.data;
  } catch (error) {
      console.error('Erreur lors de la récupération des positions des équipements:', error);
      throw error;
