...
<template>
  ...
  <v-app id="application">
    <sc-spinal-navigator class="ma-1 pa-1"
                         ...
                         :path.sync="path"
                         :max-depth="2"
                         :expand-selector="expand"></sc-spinal-navigator>
    <v-main v-if="loading">
      <sc-loader></sc-loader>
    </v-main>
    <v-main style="position: absolute; margin-top: 3.5%; width: 100%"
            v-else>
      <sc-bar-card id="bar-section"
                   :title="'Consommation d\'eau'"
                   :labels="barChartData.labels"
                   :datasets="barChartData.datasets"></sc-bar-card>

      <v-row id="stat-section">
        <v-col>
          <sc-double-stat-card :first-value="globalWaterConsumption('year')"
                               :first-unit="'L'"
                               ...
                               :second-color="'#29a0b1'"></sc-double-stat-card>
        </v-col>
        <v-col>
          <sc-double-stat-card :first-value="globalWaterConsumption('week')"
                               :first-unit="'L'"
        ...
        <v-col>
          <sc-double-stat-card :first-value="globalWaterConsumption('day')"
                               :first-unit="'L'"
  ...
  computed: {
    ...
    percentageCalculator() {
      ...
      if (this.globalWaterConsumption("day") !== 0)
        p[2] = (
          (this.sanitaryWaterConsumption("day") * 100) /
          this.globalWaterConsumption("day")
        ).toFixed(2);
      if (this.globalWaterConsumption("week") !== 0)
        p[1] = (
          (this.sanitaryWaterConsumption("week") * 100) /
          this.globalWaterConsumption("week")
        ).toFixed(2);
      if (this.globalWaterConsumption("year") !== 0)
        p[0] = (
          (this.sanitaryWaterConsumption("year") * 100) /
          this.globalWaterConsumption("year")
        ).toFixed(2);
      return p;
    },
    lastPeriodCompare() {
      let p = [0, 0];
      for (let i = -8; i >= -14; i--)
        p[1] += this.selectedSeries["month"][0][31 + i];
      if (p[1] != 0)
        p[1] = (
          ((this.globalWaterConsumption("month") - p[1]) / p[1]) *
          100
        ).toFixed(2);
      else p[1] = Infinity;
      ...
      if (lastYearConsumption != 0)
        p[0] = (
          ((currentYearConsumption - lastYearConsumption) /
            lastYearConsumption) *
          100
        ).toFixed(2);
      else p[0] = Infinity;
