...
export async function getMonthTimeSeriesAsync(
  ...
  const result = await HTTP.get(
    `endpoint/${endpointId}/timeSeries/readCurrentMonth`
  );
  return result.data;
}
...
export async function getWeekTimeSeriesAsync(
  ...
  const result = await HTTP.get(
    `endpoint/${endpointId}/timeSeries/read/${weekBegin}/${end}`
  );
  return result.data;
}
...
export async function getWorkflowListAsync(buildingId: string) {
  const result = await HTTP.get(`workflow_list`);
...
export async function getProcessListAsync(
  ...
  const result = await HTTP.get(
    `workflow/${workflowId}/process_list`
  );
  return result.data;
}
