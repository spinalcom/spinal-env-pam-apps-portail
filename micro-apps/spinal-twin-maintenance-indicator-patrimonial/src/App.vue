            ...
            <div style="width: 20%">
              ...
              >
                <template v-slot:selection="{ item, index }">
                  <v-chip
                    @click:close="building = building.filter((w) => w !== item)"
                    close
...
export default {
  ...
  computed: {
    ...
    currentBuilding() {
      return !this.building.length
        ? patrimoine.buildings
        : patrimoine.buildings.filter((b) => this.building.includes(b.name));
    },
    currentWorkflow() {
      const workflows = this.displayed.filter(
        (d) =>
          this.currentBuilding.map((cb) => cb.id).includes(d.buildingId) &&
          !d.workflowId
      );
      return !this.workflow.length
        ? workflows
        : workflows.filter((d) => this.workflow.includes(d.name));
    },
  },
  ...
  methods: {
    ...
    expandSelector(item) {
      switch (item.level) {
        case 0:
          return patrimoine.buildings.map((b) => b.name);
        case 1:
          return [
            ...new Set(
              this.displayed
                .filter((w) =>
                  this.currentBuilding.map((b) => b.id).includes(w.buildingId)
                )
                .filter((w) => config.workflowList.includes(w.name))
                .map((w) => w.name)
            ),
          ];
        case 2:
