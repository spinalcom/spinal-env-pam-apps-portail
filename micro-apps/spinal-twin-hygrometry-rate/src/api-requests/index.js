 ...
 */
 export async function getBuildingAsyncV2() {
  ...
  let promise = building.children.map(async currentFloor => {
    ...
    const result = await HTTP.get(`/building/${buildingId}/floor/${currentFloor.id}/room_list`);
    body = result.data;
    body.map(currentRoom => {
      var foundIndex = building.children.findIndex(x => x.id==currentFloor.id);
...
export async function getControlEndpointsByNameAsync(cp, building) {
  ...
  if(cp && building) {
    await getSoloCpAsync(building.id, cp)
    .then((v) => {
      if(v){
        cpTable.data.push({name: building.name, id: building.id, currentValue: v.currentValue, floor: '-'});
      }
    });
    ...
    for (let i = 0; i < building.children.length; i++){
      await getSoloCpAsync(building.children[i].id, cp).then(async(v) => {
        if(v){
          cpTable.data.push({name: '-', id: building.children[i].id, currentValue: v.currentValue, floor: building.children[i].name});
          for(let j = 0; j< building.children[i].children.length; j++) {
            await getSoloCpAsync(building.children[i].children[j].id, cp).then((v1) => {
              if(v1 && v1.currentValue == 0)
                cpTable.maintainance +=1;
              else if(v1){
                cpTable.equiped += 1;
              cpTable.data.push({name: building.children[i].children[j].name, id: building.children[i].children[j].id, currentValue: v1.currentValue, floor: building.children[i].name});
              }
