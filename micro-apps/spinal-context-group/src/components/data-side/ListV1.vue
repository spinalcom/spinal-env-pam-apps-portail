<template>
  ...
  >
      ...
      <template #append>
        <div class="d-flex align-center" height="100%">
          <v-tooltip top>
            <template v-slot:activator="{ on, attrs }">
              <v-icon
                data-cy="listV1AddButton"
                v-bind="attrs"
                v-on="on"
                :disabled="
                  typeGroup > 0 &&
                  selectedItemsGlobal[typeGroup - 1] === undefined
                "
                @click="createEntity"
                color="primary"
              >
    ...
    <v-list dense height="40vh">
      <div
        class="helper"
        v-if="itemsFiltered.length === 0 && searchModel.length === 0"
      >
      ...
      <v-list-item-group v-model="selectedItem" color="primary">
            ...
            <v-card>
              ...
              <v-card-actions>
                ...
                <v-btn
                  color="primary"
                  text
                  @click="openCloseDeleteModale(false, itemToUpdate)"
                  >Fermer</v-btn
                >
                <v-btn
                  color="primary"
                  @click="deleteEntity(itemToUpdate)"
                  data-cy="listV1DialogDeleteConfirmation"
                  >Oui</v-btn
                >
                <v-spacer></v-spacer>
              </v-card-actions>
        ...
        <v-dialog v-model="dialogUpdate" width="40vw">
          <v-card>
            <v-card-title class="text-h5"
              >Modification du groupe : "{{ itemToUpdate.title }}"
            </v-card-title>
            <v-card-text>
              <v-container>
                <v-text-field
            ...
            <v-card-actions>
              ...
              <v-btn
                color="primary"
                text
                @click="openCloseUpdateModale(false, itemToUpdate)"
                >Fermer</v-btn
              >
              <v-btn
                color="primary"
                @click="updateEntity(itemToUpdate, newTitle)"
                >Sauvegarder</v-btn
              >
              <v-spacer></v-spacer>
            </v-card-actions>
