                    ...
                    <div class="header">
                        <div class="title-wrapper">
                            ...
                            <div class="subtitle-wrapper" v-if="grpRoomFocus === 'GrpRoom'">
                                <div class="subtitle">
                                    contient <span class="bold-element">{{
            grpRoomWithChildren.groupToDisplay.filter(room =>
                room.display).length
        }}</span> pièce<span>s</span>
                                </div>
                            </div>
                        </div>
                    </div>
                            ...
                            <div class="title">
                                ...
                                <div class="tooltip">
                                    <v-tooltip top>
                                        ...
                                        <span v-if="grpRoomFocus === 'GrpRoom'"><span class="bold-element">{{
            currentGrpRoom.title }}</span> est un <span class="bold-element">groupe
                                                de
                                                pièce</span> qui appartient à la categorie ...</span>
                                        <span v-if="grpRoomFocus === 'GrpRoomList'">Le composant Assignation des
                                            espaces
                                            permet de: <br /> - visualiser des groupes
                                            de pièces <br /> - assigner des pièces à des groupes de pièces</span>
                                    </v-tooltip>
                                </div>
                            </div>
                            <div class="subtitle-wrapper" v-if="grpRoomFocus === 'GrpRoom'">
                        ...
                        <div class="info-wrapper">
                            <div class="area-wrapper">
                                <div class="initial-area area" :class="globalKpiClass">
                                    <div class="value">{{ grpRoomFocus === 'GrpRoomList' ?
            grpRoomWithChildren.globalArea.toFixed(2) : currentGrpRoom.area.toFixed(2)
                                        }}</div>
                                    <div class="unit"> m2</div>
                                </div>
                                <div class="separator-wrapper" v-if="grpRoomWithChildren.totalGain.value !== 0">
                                    <div class="separator"></div>
                                </div>
                                <div class="new-area area" v-if="grpRoomWithChildren.totalGain.value !== 0">
                                    <div class="value">{{ grpRoomWithChildren.totalArea.value.toFixed(2) }}</div>
                                    <div class="unit"> m2</div>
                                </div>
                            </div>
                            <div class="gain-wrapper">
                                <div class="arrow">
                                    <v-icon color="green"
                                        v-if="grpRoomWithChildren.totalGain.percentage > 0">mdi-arrow-up-thick</v-icon>
                                    <v-icon color="red"
                                        v-else-if="grpRoomWithChildren.totalGain.percentage < 0">mdi-arrow-down-thick</v-icon>
                                </div>
                                <div class="text"
                                    v-if="grpRoomWithChildren.totalGain.percentage > 0 || grpRoomWithChildren.totalGain.percentage < 0">
                                    <span v-if="unitMode === 'm2'">
                                        {{ grpRoomWithChildren.totalGain.value.toFixed(2) }} m2
                                    </span>
                                    <span v-else-if="unitMode === 'percentage'">
                                        {{ grpRoomWithChildren.totalGain.percentage.toFixed(2) }} %
                                    </span>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="actions">
                            <div class="search-wrapper">
                                <v-autocomplete multiple v-if="grpRoomFocus === 'GrpRoom'" :label="searchBarPlaceholder"
                                    :items="availableRooms" item-text="name" item-value="dynamicId" single-line
                                    hide-details="true" outlined dense v-model="searchModel" :loading="isLoading">
                                    ...
                                    <template v-slot:selection="{ attr, on, item, selected, index }">
                                        <v-chip v-if="index < 3" v-bind="attr" :input-value="selected" label
                                            ...
                                            :text-color="createColorPropChipsBefore(item, 'text')" deletable-chips
                                            class="white--text ma-1" close v-on="on"
                                            @click:close="deleteChipFromRoomSelection(item)">
                                            <span v-text="item.name"></span>
                                        </v-chip>
                                        <span v-if="index === 3" class="grey--text text-caption">
                                            (+{{ searchModel.length - 3 }} autre<span
                                                v-if="searchModel.length - 3 > 1">s</span>)
                                        </span>
                                    </template>
                                    <template v-slot:item="{ item }">
                                        <v-list-item-content class="d-flex inline chips-legend-container">
                                            <span v-if="grpRoomFocus === 'GrpRoom'"
                                                :style="createStyleTitleSelectBefore(item)" class="chip-legend-item">
                                            </span>
                                ...
                                <v-row>
                                    <v-col lg="8" md="8" sm="8">
                                        <v-text-field v-if="grpRoomFocus === 'GrpRoomList'" class="" dense
                                            v-model="searchModelGrp" label="Rechercher un groupe de pièce"
                                            placeholder="" outlined hide-details="true" single-line></v-text-field>
                                    </v-col>
                                </v-row>
                            </div>
                ...
                <div class="footer-wrapper">
                    <div class="footer">
                        <div class="toggle-group">
                            <ToggleButtonV1 color1="black" color2="white" symbol-left="mdi-tape-measure"
                                symbol-right="mdi-percent" v-on:toggled="toggleUnitMode">
                            </ToggleButtonV1>
                            <ToggleButtonV1 color1="black" color2="white" symbol-left="mdi-grid"
                                v-if="viewMode === 'grid'" symbol-right="mdi-radar" ref="radar"
...
// * Classes
import { GroupContextApi, NodeAttributsAPI } from '../../../services/spinalAPI';
import { GroupRoomWithChildren } from '../../../services/GroupeWithChildren/Core/GroupRoomWithChildren'
import { SpinalAPI } from '../../../services/spinalAPI';
...
// * Factory
import { iGroupRoomItemFactory } from '../../../services/GroupeWithChildren/Interfaces/IGroupRoomItem';
import { ExpansionMode, iListObjFactory } from '../../../interfaces/';

// * Plot
import { Line, Radar } from 'vue-chartjs';
import { Chart as ChartJs, Title, Legend as LegendChart, Filler, LegendElement, RadialLinearScale, PointElement, LineElement } from 'chart.js';


// * Types
import { EGroupType, GroupRoomFocus, OperationMapWithId } from '../../../services/GroupeWithChildren/Interfaces/';
...
ChartJs.register(Filler, LegendChart, RadialLinearScale, LineElement, PointElement, Title)
...
export default {
    ...
    data() {
        ...
        const grpRoomWithChildren = ref(new GroupRoomWithChildren(apiGrpContext, apiNodeAttributs, apiRoomGroup));
    ...
    methods: {
        ...
        loadData(categorieId?: number) {
            ...
            this.grpRoomWithChildren.loadData()
                .then(() => this.grpRoomWithChildren.reloadGroupToDisplay())
                .then((grps) => {
                    this.data = grps;
                    this.roomListAndObj = this.grpRoomWithChildren.rooms;
                    this.categoriesModel = this.grpRoomWithChildren.currentCategory;
        ...
        toggleComponentMode(context: 'item' | 'actions' | 'func') {
            try {
                ...
                this.grpRoomFocus = this.grpRoomFocus === 'GrpRoomList' ? 'GrpRoom' : 'GrpRoomList';
        ...
        commitChangeGrpRoom() {
            this.grpRoomWithChildren.commitChangeGrpRoom().then(() => {
        ...
        selectItem(item: IGroupRoomItem, context: 'item') {
            this.updateDataTableHeight();
            this.grpRoomWithChildren.selectItem(item);
            if (this.grpRoomFocus !== 'GrpRoomList' && context === 'item') return
            this.searchModel = '';
            this.toggleComponentMode(context);
            this.grpRoomWithChildren.setFocus(this.grpRoomFocus);
            this.currentGrpRoom = this.grpRoomWithChildren.getSelected();
            this.data = this.grpRoomWithChildren.reloadGroupToDisplay();
            this.updateFocus();
        },
        goBack() {
        ...
        reset() {
            this.grpRoomWithChildren.reset().then(() => {
                this.reloadData();
            }).catch((err: any) => {
                console.error(err);
            })
        ...
        commitChange() {
            this.grpRoomWithChildren.commitChange().then(() => {
                this.goBack();
            }).catch((e: any) => {
                console.error(e);
            })
        ...
        getOperationsItem(item: IGroupRoomItem, op: TypeLegend) {
            ...
            value = item.children?.reduce((acc: number, el: IGroupRoomItem) => acc + (el.operations === op ? 1 : 0), 0);
            if (op === 'ToAssign') {
                value += item.children?.reduce((acc: number, el: IGroupRoomItem) => acc + (el.operations === 'ToReAssign' ? 1 : 0), 0);
            }
            return value;
        },
        guessItemColor(item: IGroupRoomItem) {
            return GroupRoomWithChildren.guessItemColor(item.operations);
        },
        getFutureOperation(item: IGroupRoomItem) {
            const ops = this.grpRoomWithChildren.getFutureOperation(item);
            return GroupRoomWithChildren.guessItemColor(ops);
        },
        addRoom(ctx: 'search' | '', item?: Partial<IGroupRoomItem | any>) {
            ...
            if (!item) {
                for (const id of this.searchModel) {
                    pms.push(this.grpRoomWithChildren.addItemFromViewer(ctx, iGroupRoomItemFactory.build({ id: id })));
                }
            }

            Promise.all(pms).then().then(() => {
                this.reloadData();
            }).catch((err: any) => {
                console.error(err);
            })
        ...
        deleteEntity(item: IGroupRoomItem, context: 'menu' | 'actions') {
            this.grpRoomWithChildren.deleteItem(item).then(() => {
                this.reloadData()
            }).catch((err: any) => {
                console.error(err);
            })
        ...
        restore(item: IGroupRoomItem) {
            this.grpRoomWithChildren.restoreItem(item).then(() => {
        ...
        createStyleTitleSelectBefore(item: any) {
            ...
            let futureOp = this.grpRoomWithChildren.getFutureOperation(item);
            let styleObj = {};

            styleObj["height"] = "2em";
            if (this.grpRoomWithChildren.selectedGrpRoom.children.find((room) => room.title === item.name)) {
                styleObj["background"] = GroupRoomWithChildren.guessItemColor("Assigned");
                return styleObj;
            }
            switch (futureOp) {
                case 'ToAssign':
            ...
            itemColorBefore = GroupRoomWithChildren.guessItemColor(LegBefore);
        ...
        createColorPropChipsBefore(item: any, ctx = 'bg'): string {
            ...
            if (this.grpRoomWithChildren.selectedGrpRoom.children.find((room) => room.title === item.name)) {
                itemColorBefore = GroupRoomWithChildren.guessItemColor("Assigned");
                textColor = 'white';
        ...
        switchCategory(category: number | IGroupRoomItem) {
            ...
            if (category && typeof category === 'number') {
                this.grpRoomWithChildren.switchCategories(category).then((newCategories) => {
                    this.reloadData()
                    this.isLoadingData = false;
                }).catch((err: any) => {
                    console.error(err);
                ...
                this.grpRoomWithChildren.switchCategories(category.id).then((newCategories) => {
                    this.reloadData()
                    this.isLoadingData = false;
                }).catch((err: any) => {
                    console.error(err);
    ...
    computed: {
        availableRooms() {
            try {
                ...
                const rooms = this.grpRoomWithChildren.rooms.list;
                const floorRooms = this.grpRoomWithChildren.rooms.list.filter((x: IGroupRoomItem) => x.floorId === zoneSelected.dynamicId)
        ...
        searchBarPlaceholder() {
            return this.grpRoomFocus === 'GrpRoomList' ? 'Recherchez un groupe de pièces' : 'Selectionner une ou plusieurs pièce(s)';
        },
        globalKpiClass() {
            if (this.grpRoomWithChildren.totalGain.value !== 0) {
        ...
        dataFiltered() {
            if (this.grpRoomFocus === 'GrpRoom') {
                ...
                return this.data?.filter((grp) => grp?.title.toLowerCase().startsWith(this.searchModelGrp.toLowerCase()));
            }
            return []
        },
        ...
        dataRadar() {
            ...
            if (this.grpRoomFocus === 'GrpRoomList') {
                areaAfter = this.data.map((grpGroup: IGroupRoomItem) => grpGroup.newArea);
