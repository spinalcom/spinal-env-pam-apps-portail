<template>
  ...
  >
    ...
    <template v-slot:header.data-table-select="{}">
      <v-simple-checkbox
        @click="selectRows('all', 'dynamicId')"
        v-if="showSelect"
    ...
    <template v-slot:item="{ item }">
      <tr>
        <td class="d-flex min-width" v-if="showSelect">
          <v-simple-checkbox
            v-model="lexiconShowSelect[item.dynamicId]"
            @click="selectRows('single', 'dynamicId')"
          >
          </v-simple-checkbox>
        </td>
  ...
  methods: {
    ...
    recomputeSelectedItems(tag: string) {
      ...
      for (const item of this.items) {
        // !! Source of Bug
        if (
          this.lexiconShowSelect[item?.[tag]] &&
          this.lexiconShowSelect[item?.[tag]] === true
        ) {
