...
export type IViewInfoRes = {
	dynamicId: number;
	data: IViewInfoItemRes[];
};

export type IViewInfoItemRes = {
	bimFileId: string;
	dbIds: number[];
};
export type IViewInfoTmpRes = {
	bimFileId: string;
	dbIds: Set<number>;
};
...
export async function getViewInfo(
  ...
  const url = spinalAPI.createUrlWithPlatformId(
	...
	const result = await spinalAPI.post<IViewInfoRes[]>(url, options);
	return result.data;
}
