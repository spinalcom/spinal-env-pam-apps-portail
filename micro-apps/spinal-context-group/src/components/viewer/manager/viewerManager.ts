...
import {
  getViewInfo,
  getViewInfoFormatted,
  type IViewInfoBody,
  type IViewInfoItemRes,
  type IViewInfoTmpRes,
  mergeIViewInfo,
} from "../requests/GeographicContext/getViewInfo";
import {
  type IPlayload,
  type IPlayloadWithComponent,
} from "../interfaces/IPlayload";
import {
...
export class ViewerManager {
  ...
  private readonly _buildingInfo: any = {};
  private readonly _viewerStores = {};
  private _viewerStartedList: Record<string, Set<string>> = {};
      ...
      Autodesk.Viewing.Initializer(options, () => {
        if (viewerDiv) {
          ...
          const viewer: Autodesk.Viewing.Viewer3D =
            new Autodesk.Viewing.Viewer3D(viewerDiv, customProfileSettings);
          _self.viewer = viewer;
          resolve(viewer);
        }
      });
  ...
  public async loadInViewer(
    item: IPlayload,
    loadOnlyThisModel = true,
    body?: IViewInfoBody & {
      dbIdsToAdd?: Array<{ bimFileId: string; dbIds: number[] }>;
    }
  ) {
    ...
    emitter.once(VIEWER_INITIALIZED, async () => {
      ...
      emitter.once(VIEWER_EVENTS.LOADED as any, (data) => {
  ...
  public async getViewerInfoMerged(
    ...
    return res.map(
      (it: IViewInfoTmpRes): IViewInfoItemRes => ({
  ...
  public async getViewerInfo(
    argItem: IPlayload | IPlayload[],
    argBuildingId?: string,
    body?: IViewInfoBody
  ): Promise<any[]> {
    ...
    for (const dynId of ids) {
      if (this._viewerStores.GET_VIEWER_INFO[dynId]) {
        const itemData = (
          await this._viewerStores.GET_VIEWER_INFO[dynId].next()
        )?.value;
        if (itemData) {
          res.push(itemData);
        }
  ...
  public async select(item: IPlayload) {
    console.log("On call la fct select");
    return this._fctViewerIteract(VIEWER_OBJ_SELECT, item);
  }

  public async isolate(item: IPlayload) {
  ...
  public async fitToView(item: IPlayload) {
    return this._fctViewerIteract(VIEWER_OBJ_FIT_TO_VIEW, item);
  }

  public async unload(item: (IPlayload | string) | Array<IPlayload | string>) {
  ...
  public async addComponentAsSprites(
    item: IPlayloadWithComponent | IPlayloadWithComponent[],
    buildingId: string,
    component?: Vue
  ) {
    const formatted = await this._getAndFormatViewerInfos(
      item,
      buildingId,
      component
    );
    const emitter = EmitterViewerHandler.getInstance();
    emitter.emit(
      VIEWER_EVENTS.VIEWER_ADD_COMPONENT_SPRITE as any,
      formatted as any
    );
  }
  ...
  private async _getAndFormatViewerInfos(
    item: IPlayloadWithComponent | IPlayloadWithComponent[],
    buildingId?: string,
    component?: Vue
  ) {
    ...
    return item.map((i) => ({
      ...
      modelId:
        (i as IPlayload).floorId ||
        (i as IPlayload).id ||
        (i as IPlayload).dynamicId,
      parent: { ...i },
  ...
  private async _fctViewerIteract(
    eventName: keyof ViewerEventWithData,
    playload: (IPlayload | string) | Array<IPlayload | string>
  ): Promise<any> {
    const emitter = EmitterViewerHandler.getInstance();
    ...
    const data: IViewInfoItemRes[] = await this.getViewerInfoMerged(
      playload as IPlayload
    );
    const res = data.map((it) => ({
      dbIds: it.dbIds,
      bimFileId: it.bimFileId,
      modelId:
        (playload as IPlayload).floorId ||
        (playload as IPlayload).id ||
        (playload as IPlayload).dynamicId,
    }));
  ...
  private _addViewLoaded(nodeId: string, models: any[]) {
    if (!this._viewerStartedList[nodeId]) {
      this._viewerStartedList[nodeId] = new Set([]);
    }
