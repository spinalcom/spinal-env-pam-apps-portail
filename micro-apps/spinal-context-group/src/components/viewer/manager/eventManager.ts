  ...
  listenAllEvents(viewer: Autodesk.Viewing.Viewer3D): Promise<() => void> {
    return new Promise((resolve) => {
      ...
      emitterHandler.on(VIEWER_START_LOAD_MODEL, async (data: any) => {
        ...
        emitterHandler.emit(<any>VIEWER_EVENTS.LOADED, {
          id: data.item.dynamicId,
          models,
        });
      });
      ...
      emitterHandler.on(VIEWER_OBJ_ISOLATE, (data: any) => {
        if (data && data.length > 0)
          return viewerUtils.viewerIsolation(viewer, data);
      ...
      emitterHandler.on(
        VIEWER_SET_CAMERA,
        (data?: EViewerSetCamera | EViewerSetCamera[]) => {
      ...
      function viewerGetSelectionChange(data: any) {
        ...
        for (const selection of data.selections) {
          eventData.push({
            ...
            modelId: ModelManager.getInstance().getSpinalModelID(
              selection.model.id
            ) as any,
          });
        }
