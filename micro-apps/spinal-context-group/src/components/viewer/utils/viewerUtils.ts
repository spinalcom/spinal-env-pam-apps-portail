...
import {
	EViewerSetCamera,
	type IDbIdModelAggregate,
	type IStartLoadModel,
	SceneAlignMethod,
} from 'spinal-viewer-event-manager';
import {type IloadModelTask} from '../interfaces/ILoadModelTask';
...
import {type IViewerColorData} from '../interfaces/IViewerColorData';
import {getPosition} from './getObjectPos';
import SpriteManager from '../manager/spriteManager';

export class ViewerUtils {
	private static _instance: ViewerUtils;
	private _isFirstModel = true;
	public waitBeforeDisplaySprites = true;

	private constructor() {}

	public static getInstance(): ViewerUtils {
	  this._instance ||= new ViewerUtils();
	...
	public async load3DModels(
	  ...
	  for (const d of data.models) {
		  ...
		  offset: d.offset?.[Object.keys(d.offset)[0]],
		  id: d.id,
		  name: d.name,
			});
	  }
	  ...
	  return load1stThenAll(
			tasks,
			async (d: IloadModelTask): Promise<Autodesk.Viewing.Model> => this._loadBimFile(
				viewer,
	...
	public async unloadModel(
	  ...
	  if (!arg_modelIds || arg_modelIds.length === 0) {
			return;
		}
	...
	public viewerIsolation(
	  viewer: Autodesk.Viewing.Viewer3D,
	  data: Array<IDbIdModelAggregate & {bimFileId: string}>,
	): void {
	  this.setWaitBeforeDisplaySprites(true);
	  ...
	  function isolateAll(m: Autodesk.Viewing.Model) {
		  ...
		  const dbidRoot = tree.nodeAccess.dbIdToIndex[rootId];
	...
	public viewerFitToView(
	  ...
	  const arrayToFit: Array<{model: Autodesk.Viewing.Model; selection: number[]}>
	...
	public setCamera(
	  ...
	  if (viewcuiext) {
			viewcuiext.setViewCube(dataFormatted.join(', '));
		}
	}
	...
	public async addComponentAsSprite(
	  ...
	  return Promise.all(promises).then(async result => SpriteManager.getInstance().addComponentAsSprite(viewer, result));
	}
	...
	private async _loadModel(
	  ...
	  return new Promise((resolve, reject) => {
		  ...
		  if (m && e.model.id === m.id) {
			...
			const fct = start ? viewer.start : viewer.loadModel;
			fct.call(
		  viewer,
		  path,
		  option,
	...
	private async _waitModelIsLoading(): Promise<boolean> {
		return new Promise((resolve, reject) => {
			const wait = () => {
				if (this.waitBeforeDisplaySprites && !this._allModelsAreLoaded()) {
					setTimeout(() => {
						wait();
					}, 500);
				} else {
	...
	private _classifyDbIdsByModel(
	  ...
	  const list: Array<{model: Autodesk.Viewing.Model; dbIds: number[]}> = [];
