...
import {type AttributeCategory, type Attribute, type NodeAttributs} from '../DTO';
...
class NodeAttributsAPI extends AAPI {
	...
	public async getAttributesMultipleNode(nodeDynamicIds: number[]): Promise<NodeAttributs[]> {
		...
		return new Promise<NodeAttributs[]>((resolve, reject) => {
				url = this._spinalApi?.createUrlWithPlatformId(super.getIdCurrentBuilding(), '/api/v1/node/attribute_list_multiple');
				requestPromise = this._spinalApi?.post(url, [...nodeDynamicIds]) as Promise<any>;
	...
	// In order to prevent merge conflict i will not write this function into spinalAPI
}
