<template>
  ...
  >
    ...
    <div
      v-show="vSelectedTab === 'Equipements'"
      style="padding: 2px; margin-top: 20px;"
...
export default {
  ...
  mounted() {
    ...
    this.dynamicHeader = this.dynamicHeaders();
  },
  computed: {
     dataStore() {
     ...
     getDynamicHeader() {
      return this.$store.state.appDataStore.dynamicHeader;
     },
    filteredContexts() {
      if (this.contexts) {
    ...
    getDynamicHeader() {
      ...
      this.dataStore.map((item) =>{
        item.attributes.map((attr) => {
          const header = this.dynamicHeader.find((header) => header.parentName === attr.name);
          if(header) {
            const value = attr.attributs.find((attr) => attr.label === header.text);
            if(value) {
              values.push({
                label: value.label,
                value: value.value,
      ...
      values.forEach((value) => {
        const formattedKey = value.label.replace(/\s+/g, '_');
        const existingItem = this.dataStore.find(
          (item) => item.dynamicId === value.dynamicId
        );

        if (existingItem) {
          existingItem[formattedKey] = value.value;
        }
      });
    },
