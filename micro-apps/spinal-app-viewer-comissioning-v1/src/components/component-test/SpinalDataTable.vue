<template>
  <div>
    <FormBreadCrumb
      v-if="showForm"
      :title="getTitleList"
      :items="getListItemofcontext"
      :typeList="getTypeList"
      @item-selected="ItemSelected"

    />
<div v-else 
    style="background-color: white; padding: 10px; border-radius: 5px; margin-top: 10px; margin-bottom: 10px; position: relative;">
    
    <div class="regex_config">
    <v-btn color="#14202C" 
    @click="showdialog = !showdialog"
    dense
    style="color: white; font-size: 14px !important; font-family: Charlevoix !important;"
    
    >Configurer la Regex</v-btn>

    <FormToChangeColumn v-if="showdialog" @close="showdialog = false" :columns="getHeaders" />
  </div>
    ...
    <v-data-table
      ...
      v-slot:[`header.${header.value}`]="{ header }"
        ...
        <td v-if="hasActions" :class="{ colortd: selected_id === item.dynamicId }">
          <div v-if="item.type==='BIMObject'" style="display:flex;  justify-content: center; gap:4px  ">
      ...
      <template v-if="isUrl(getAttributeValue(item, header.value))">
        <a :href="getAttributeValue(item, header.value)" target="_blank">
          {{ getAttributeValue(item, header.value) }}hehzezizh
        </a>
      </template>
      <template v-else-if="hasFilter">
        <div style="display: flex; align-items: center; gap: 5px; justify-content: center;">
          <div style="width: 15px; height: 20px; border-radius: 5px; background-color: red;"></div>
         <span>{{ getAttributeValue(item, header.value) }}</span>
...
import FormBreadCrumb from "./FormBreadCrumb.vue";
import TreeViewSelect from "./TreeViewSelect.vue";
import FormToChangeColumn from "./Form-to-change-column.vue";
export default {
  props: [
    'items',
    'headers',
  ...
  computed: {
    ...
    hasFilter() {
      const filter = this.$store.state.appDataStore.regex_filter;
      return this.headers.some((header) => header.text === filter.column);
    },
    ...
    getListItemofcontext() {
      console.log('getListItem', this.$store.state.appDataStore.user_selection_list);
      return this.$store.state.appDataStore.user_selection_list;
    },
    getTypeList() {
      return this.$store.state.appDataStore.type_list;
    },

    showForm() {
      return this.$store.state.appDataStore.showForm;
    },

    getTitleList() {
      return this.$store.state.appDataStore.title_list;
    },
     getHeaders() {
      const columns = this.headers.map(header => header);
...
async mounted() {
    this.context = await this.getContextList();
  ...
  methods: {
   ...
   async getContextList (){
    ...
    dispatchObject.forceUpdate = true;
       ...
       this.contextList = await this.$store.dispatch(ActionTypes.GET_EQUIPMENTS_GROUP, dispatchObject);
    ...
    const dispatchObject = {
      buildingId: localStorage.getItem("idBuilding"),
      patrimoineId: JSON.parse(localStorage.getItem("patrimoine")).id,
      contextDynId: this.selectedItem.item.dynamicId
    };
    dispatchObject.forceUpdate = true;
        this.contextList = await this.$store.dispatch(ActionTypes.GET_CATEGORY_LIST, dispatchObject);
          console.log('contextList', this.contextList);
          this.$store.commit(MutationTypes.SET_USER_SELECTION, this.contextList);
          this.$store.commit(MutationTypes.SET_TYPE_LIST, 'cat');
          console.log('user_selection_list', this.$store.state.appDataStore.user_selection_list);
    ...
    async getGroupsList() {
      ...
      const dispatchObject = {
        ...
        categoryDynId: this.selectedItem.item.dynamicId
      }
      dispatchObject.forceUpdate = true;
      this.contextList = await this.$store.dispatch(ActionTypes.GET_GROUP_LIST, dispatchObject);
      console.log('contextList', this.contextList);
      this.$store.commit(MutationTypes.SET_USER_SELECTION, this.contextList);
    ...
    async getItemList() {
      this.$store.commit(MutationTypes.SET_SHOW_FORM, false);
      const data = await this.$store.dispatch(ActionTypes.GET_ITEM_LIST, {
        buildingId: localStorage.getItem("idBuilding"),
        contextDynId: this.$store.state.appDataStore.user_selected.ctx.dynamicId,
        categoryDynId: this.$store.state.appDataStore.user_selected.cat.dynamicId,
        groupDynId: this.selectedItem.item.dynamicId,
        typeItem: this.$store.state.appDataStore.user_selected.ctx.type
      });
      this.$store.commit(MutationTypes.SET_DATA, data);
      const attributesData = await this.$store.dispatch(ActionTypes.GET_ATTRIBUTE_LIST, {
        buildingId: localStorage.getItem("idBuilding"),
        item: data
      });
      console.log('attributesData', attributesData);
      this.$store.commit(MutationTypes.SET_DATA, attributesData);
    },
