...
export enum MutationTypes {
	...
	ADD_DYNAMIC_HEADER = "SET_DYNAMIC_HEADER",
	DELETE_DYNAMIC_HEADER = "DELETE_DYNAMIC_HEADER",
	SET_REGEX_FILTER = "SET_REGEX_FILTER"
}
...
export type MutationsAppData<S = StateAppData> = {
	...
	[MutationTypes.SET_TYPE_LIST](state: StateAppData, data: string): void;
	[MutationTypes.SET_SHOW_FORM] (state: StateAppData, payload: boolean): void;
	[MutationTypes.SET_TITLE_LIST] (state: StateAppData, payload: string): void;
	[MutationTypes.ADD_DYNAMIC_HEADER](state: StateAppData, payload: IDynamicHeader): void;
	[MutationTypes.DELETE_DYNAMIC_HEADER](state: StateAppData, payload: IDynamicHeader): void;
	[MutationTypes.SET_REGEX_FILTER](state: StateAppData, payload: IRegexFilter): void;
...
export const mutations: MutationTree<StateAppData> & MutationsAppData = {
	...
	[MutationTypes.SET_USER_SELECTED](state: StateAppData, payload: { key: 'cat' | 'grp' | 'ctx'; value: any, type: string }): void {
		if (typeof state.user_selected !== 'object') state.user_selected = {};
		// Pour 'cat' et 'ctx' et 'grp', garder le comportement existant
		state.user_selected = { ...state.user_selected, [payload.key]: payload.value, type: payload.type };
	},
	...
	[MutationTypes.ADD_DYNAMIC_HEADER](state: StateAppData, payload: IDynamicHeader): void {
		state.dynamicHeader.push(payload);
	},

	[MutationTypes.DELETE_DYNAMIC_HEADER](state: StateAppData, payload: IDynamicHeader): void {
		console.log("payload", payload);
		state.dynamicHeader = state.dynamicHeader.filter((header) => header.text !== payload.text);
	},

	[MutationTypes.SET_REGEX_FILTER](state: StateAppData, payload: IRegexFilter): void {
		state.regex_filter = payload;
	}
