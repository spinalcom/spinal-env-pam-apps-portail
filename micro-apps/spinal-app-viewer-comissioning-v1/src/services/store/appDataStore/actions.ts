...
import { getGroupContextCategoryList, getGroupContextGroupList, getGroupContextread, getItemList } from "../../spinalAPI/ContextGroup/groupContext";

import { createAttribut, createCategory, getCategoriesList } from "../../spinalAPI/NodeAttributs/nodeAttributs";

import { getContext, getCategoryList, getGroupList, getequipementList } from "../../spinalAPI/EquipementsGroup/equipementsGroup";
...
import { getAttributeList } from "../../spinalAPI/attribute/attributelist";

const ApiIteratorStore: ApiIteratorStoreType & ApiIteratorStoreRecordStringType & ApiIteratorStoreRecordNumberType = {};

export const actions = {

	async [ActionTypes.GET_ITEM_LIST]({ commit }: AugmentedActionContextAppData, { buildingId, contextDynId, categoryDynId, groupDynId, typeItem }: { buildingId: string; contextDynId: number, categoryDynId: number, groupDynId: number, typeItem: string}): Promise<any> {
		const spinalAPI = SpinalAPI.getInstance();
		try {
			const result = await getItemList(buildingId, contextDynId, categoryDynId, groupDynId, typeItem);
...
async [ActionTypes.GET_ATTRIBUTE_LIST]({ commit }: AugmentedActionContextAppData, { buildingId, item }: { buildingId: string; item: any[] }): Promise<any> 
{
	try {
		console.log('item', item);
		const response = await getAttributeList(buildingId, item);
		console.log('response in action attributes List: ', response);
		return response;
	} catch (error) {
	...
	async [ActionTypes.GET_EQUIPMENTS_GROUP]({ commit }: AugmentedActionContextAppData, { buildingId, patrimoineId, floorId, id, forceUpdate }: any): Promise<IZoneItem[]> {
		...
		if (typeof floorObjStore[id] === "undefined" || forceUpdate === true) {
			floorObjStore[id] = spinalAPI.createIteratorCall(getContext, patrimoineId, buildingId, floorId);
		}
		const floors = await floorObjStore[id].next();
		// commit(MutationTypes.SET_DATA, { id: id, items: floors.value });
		return floors.value;
