...
export async function getContext(patrimoineId: string, buildingId: string): Promise<IZoneItem[]> {
    const spinalAPI = SpinalAPI.getInstance();
    const url_equipement = spinalAPI.createUrlWithPlatformId(buildingId, `api/v1/equipementsGroup/list`);
    const url_room = spinalAPI.createUrlWithPlatformId(buildingId, `api/v1/roomsGroup/list`);
    let result = await spinalAPI.get<IZoneItem[]>(url_equipement);
    let result2 = await spinalAPI.get<IBuildingItem[]>(url_room);
    const data = [...result.data, ...result2.data];
    const res = data.map((obj) => {
        Object.assign(obj, { patrimoineId, buildingId, req: 'ContextList' });
        return obj;
    });
    console.log('LES DONNÉES DU CONTEXT: ', res);
    return res;
}
...
export async function getGroupList(patrimoineId: string, buildingId: string, contextDynId: number, categoryDynId: number): Promise<IZoneItem[]> {    
    ...
    console.log('LES DONNÉES DU GROUPE: ', res);
    return res;
}
