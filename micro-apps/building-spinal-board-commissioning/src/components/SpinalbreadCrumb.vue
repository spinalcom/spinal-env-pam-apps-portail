        ...
        <nav title="Modifier la séléction" @click.stop="showSelection = !showSelection" class="breadcrumbs">
            ...
            <div href="#selected_grp" class="breadcrumbs__item_chips">
                <ul style="display: flex; gap: 2px;">
                    ...
                    small 
                    close
                    @click:close="tarrr(idx)"
                    color="#14202c"
                    text-color="white"
                     v-for="(item, idx) in groupsSelect"> {{ item.name }}</v-chip>
                </ul>
            </div>
        </nav>
            ...
            <div @click.stop style="display: flex;background-color: white;border-radius: 8px;height: 500px;">
                ...
                <div
                    ...
                    <ul>
                        <div :class="{ 'selected': selected_cat === cat.name }" class="choose_li"
                            style="cursor: pointer;" v-for="cat in categoryList" :key="cat.name"
                ...
                <div
                    ...
                    <ul>
                        ...
                        <div @click="toggleSelection(grp)" :class="{ 'selected': groupsSelect.includes(grp) }"
                            class="choose_li" v-for="grp in groupList" :key="grp.name"><label style="cursor: pointer;"
                                :for="grp.name">{{
                                    grp.name }}</label></div>
...
export default {
   ...
   async mounted() {
    ...
    methods: {
        ...
        toggleSelection(group) {
           if(group)  {
            ...
            if(this.groupsSelect.includes(group)){
                this.groupsSelect.splice(this.groupsSelect.indexOf(group), 1);
            }
            else {
                this.groupsSelect.push(group);
            }
            this.$store.commit(MutationTypes.SET_GROUP_EQUIP, this.groupsSelect);
           }
        },
        validate() {
            this.$emit('loadData')
       ...
       async emitValue(listType, value) {
           if(listType === 'ctx') {
            this.contexId = value.dynamicId
