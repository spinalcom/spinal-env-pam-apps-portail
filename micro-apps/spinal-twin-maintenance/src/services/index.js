...
export function curveData(period, timestamp, domain, list, domainList) {
  ...
  if (period === "Semaine") {
    ...
    for (let i = 0; i < list.length; i++) {
      ...
      if (index != -1) {
        if (
          (list[i].step === "Attente de lect.avant Execution" || ticketList[i].step === "Raised") &&
    ...
    for (let i = 0; i < ticketList.length; i++) {
      ...
      index = exactDate.findIndex((date) => date === formattedDate);
      if (index != -1) {
        if (ticketList[i].step === "Attente de lect.avant Execution" || ticketList[i].step === "Raised") {
          for (let dp = 0; dp < domainPie.length; dp++) {
            if (domainPie[dp].label === ticketList[i].domain) {
              domainPie[dp].value += 1;
              domainExists = true;
...
export async function getData() {
  ...
  try {
    ...
    workflowlist.push(...listWorkflowResponse.data);
    let domains = [];
    ...
    const subTicket = [...finalTicketList];
    let i = 0;
    const ticketIds = subTicket.map((t) => {
      return t.dynamicId;
    })
    const _ticketListStream = []
    for (let i = 0; i < ticketIds.length; i += 50) {
      ...
      const ticketPromise = readDetailsResponse.data.map(async (t) => {
        if (t.log_list.length == 0)
          return {
            domains: t.process.name,
            name: t.userName,
            step: t.step.name,
            date: 0,
      ...
      _ticketListStream.push(...ticketPromise);
      const ticketListStream = await Promise.all(_ticketListStream);
      return {
        domains: domains,
  ...
  for (const workflow of listWorkflowResponse.data) {
    ...
    const readDetailsResponse = await HTTP.post(`building/${buildingId}/ticket/read_details_multiple`, ticketIds);
      readDetailsResponse.data.map((t) => {
       ...
       todaysCounter = finalTicketList.reduce((a, cv) => a + cv, 0);
  }
  ...
  for (const workflow of listWorkflowResponse.data) {
    ...
    const todaysTickets = await HTTP.post(
      ...
      readDetailsResponse.data.map((t) => {
          ...
          else if(t.log_list[0] && moment(t.log_list[0].date).isBetween(bd.valueOf(), ed.valueOf())) {
            finalTicketList.push(1)
          }
