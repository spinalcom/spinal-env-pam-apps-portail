  ...
  <div class="HardwareContextContainer">
    <div class="custom-dropdown">
      <div class="dropdown-selected" style="font-size: 15px!important;" @click="toggleDropdown">
        <!-- {{ selectedLabel }} -->
        {{ selectedLabel.length > 60 ? selectedLabel.substring(0, 60) + '...' : selectedLabel }}
        <span class="arrow"></span>
      </div>
      <ul v-show="dropdownOpen" class="dropdown-options" style="font-size: 15px!important;">
        <li v-for="(item, index) in visibleItems" :key="index" @click="selectItem(item.dynamicId)"
          :class="{ selected: item.dynamicId === selectedItemId }" style="font-size: 15px!important;">
          {{ item.nodes[0].name }}
...
class HardwareContext extends Vue {
  ...
  get visibleItems() {
    ...
    // console.log(this.data.slice(this.startIndex, endIndex));
  ...
  get selectedLabel() {
    ...
    return selectedItem ? selectedItem.nodes[0].name : "Select an option";
  }
