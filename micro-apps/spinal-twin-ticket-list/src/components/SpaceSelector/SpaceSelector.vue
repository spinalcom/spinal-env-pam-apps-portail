<template>
  <div class="space-selector-container" :class="{ isopen: open }">
    ...
    <v-card color="#14202C" :class="{ 'space-selector-open': open }" class="space-selector" style="
      ...
      showSign();
      " ref="SpaceSelectorTitleContainer" class="space-selector-header">
      ...
      <transition-group id="myDiv" name="staggered-fade" class="card-list spinal-scrollbar"
        :style="[{ 'overflow-y': isFill + ' !important' }]" tag="div" v-bind:css="false" v-on:before-enter="beforeEnter"
        v-on:enter="enter">
        <SpaceSelectorItem class="staggered-fade-item" v-for="(item, index) in buildingStructure"
          ...
          v-bind:data-index="index" :maxDepth="maxDepth" @onSelect="select(item, index)" :selected="selectedZone"
          :is-selected="indexSelected === index" @onOpenClose="expandCollapse(item, index)"></SpaceSelectorItem>
      </transition-group>
    </v-card>
  </div>
</template>
...
class SpaceSelector extends Vue {
  ...
  async onSelectedChange() {
    if (!this.selectedZone || !Array.isArray(this.selectedZone.parents)) {
      console.warn("selectedZone.parents is not a valid array");
      return;
    }
