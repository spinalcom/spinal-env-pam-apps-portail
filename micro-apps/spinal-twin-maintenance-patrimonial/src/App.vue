<template>
  <v-app class="spinal-font">
    ...
    <MicroApp :temporality="selectedTime" :buildings="selectedSpace" />
  </v-app>
</template>
...
class App extends Vue {
  ...
  public get selectedSpace() {
    const buildings = this.$store.state.appDataStore.buildings;
    return this.selectedZone.haveChildren
      ? buildings
      : [buildings.find((b) => b.id === this.selectedZone.staticId)];
  }
  ...
  async mounted() {
    ...
    this.defaultSelected.name = patrimoine.name;
    this.defaultSelected.dynamicId = patrimoine.id;
    this.defaultSelected.buildings = patrimoine.buildings;
    this.selectedZone = this.defaultSelected;
  }
  onTimeSelectOpen(item?: any): {
    name: string;
    staticId: string;
    dynamicId: number;
    level: number;
