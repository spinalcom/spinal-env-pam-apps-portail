...
class App extends Vue {
  ...
  load = false;
  ticketList: { domains: any[]; ticketList: any[] };
  ...
  async mounted() {
    this.ticketList = await getData(this.buildings);

    for (const domain of this.ticketList.domains)
      this.domainList.push({ name: domain.name, color: domain.color });

    this.interval();
    this.load = true;
    this.todaysTickets = await ticketsCreatedtoday(this.buildings);
  }

  interval() {
    this.currentTimestamp = {
      valueTime: (this.currentTimestamp.valueTime = moment().valueOf()),
    };
    this.spreadData();
  }
  ...
  async domainChange() {
    ...
    if (res)
      [this.barData, this.cardsData, , this.declarerPie, this.buildingPie] = [
        ...res,
      ];
  ...
  async nav(payload: number): Promise<void> {
    if (this.temporality.name == "Semaine") {
      this.currentTimestamp = {
        valueTime: moment(this.currentTimestamp.valueTime)
          .add(payload, "weeks")
          .valueOf(),
      };
    }
  ...
  async spreadData() {
    let ticketsCreatedPromise = ticketsCreated(
      this.currentTimestamp.valueTime,
      this.temporality.name,
      this.buildings
    );
  ...
  getColors(size: number): string[] {
    switch (size) {
      case 1:
        return ["#14202C"];
      case 2:
        return ["#14202C", "#ACCAE8"];
      case 3:
        return ["#14202C", "#47719B", "#ACCAE8"];
      case 4:
        return ["#14202C", "#2F4F70", "#6593C0", "#ACCAE8"];
      case 5:
        return ["#14202C", "#2F4F70", "#6593C0", "#80A9D2", "#ACCAE8"];
      case 6:
        return [
          "#14202C",
          "#2F4F70",
