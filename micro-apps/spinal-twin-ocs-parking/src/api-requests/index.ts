...
export async function getRoomGroupContextsAsync() {
  ...
  const result = await HTTP.get(`/roomsGroup/list`);
  return result.data;
}
...
export async function getRoomGroupCategoriesAsync(contextId: number) {
  ...
  const result = await HTTP.get(
    `/roomsGroup/${contextId}/category_list`
  );
  return result.data;
}
...
export async function getRoomGroupGroupsAsync(
  ...
  const buildingId = localStorage.getItem("idBuilding");
  const result = await HTTP.get(
    `/roomsGroup/${contextId}/category/${categoryId}/group_list`
  );
  return result.data;
}
...
export async function getDayTimeSeries(endpointId: number) {
  const buildingId = localStorage.getItem("idBuilding");
  const result = await HTTP.get(
    `/endpoint/${endpointId}/timeSeries/readCurrentDay`
  );
  return result.data;
}
