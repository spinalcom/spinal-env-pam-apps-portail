...
import { getBuilding, getFloors } from './services';
...
class App extends Vue {
  ...
  source : any = env.source;
  time = { name: "SEMAINE", value: 'week' }
  maxDepth = (env as any).getFloors ? 1 : -1;
  selectedFloor = '';
  ...
  async onSpaceSelectOpen(item?: ISpaceSelectorItem): Promise<any> {
    switch (item?.type) {
      ...
      case 'building':
        const floors = await getFloors(this.source, item.dynamicId);
