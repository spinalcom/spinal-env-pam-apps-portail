...
export async function getBuilding(source) {
  ...
  if (type !== ENDPOINT_TYPE) cpList = await HTTP.get(`/node/${result.data.dynamicId}/control_endpoint_list`);
  if (type !== CONTROLPOINT_TYPE) pList = await HTTP.get(`/node/${result.data.dynamicId}/endpoint_list`);

  for (const s of source) {
    if (s.type === CONTROLPOINT_TYPE && cpList.data.find(e => e.profileName === s.profile) && cpList.data[cpList.data.findIndex(e => e.profileName === s.profile)].endpoints.find(e => e.name === s.name)) {
...
export async function getFloors(cp, buildingId) {
  if (!Array.isArray(cp)) cp = [cp];
  const cpNames = cp.map(el => el.name);

  const buildingId = buildingId || localStorage.getItem('idBuilding');
  // get all floors
  ...
  return result.data.reduce(async (prom, floor) => {
    ...
    let cpList = await getControlEndpointList(floor.dynamicId);
...
function getControlEndpointList(nodeDynamicId) {
