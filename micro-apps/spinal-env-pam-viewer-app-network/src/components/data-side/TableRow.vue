<template>
  <div class="dataTable">
    <div class="selection" @click="handleClick(data.dynamicId)" :class="{
      ...
      <td class="name">
        ...
        }" :style="{ marginLeft: `${level * 15}px` }"></div>
        {{ data.dynamicId }}
      </td>

      <td class="type">
        <div class="typologie" :style="{ backgroundImage: `url(${dataImageUrl})` }"></div>
        {{ data.typologie }}
      </td>
      <!-- :style="{
    ...
    <template v-if="showChildren">
      <TableRow v-for="(child, childIndex) in data.nodes" :key="childIndex" :data="child" :fulldata="fulldata"
        :index="childIndex" :level="level + 1" :pairs="pairs" :selectedDynamicId="selectedDynamicId"
