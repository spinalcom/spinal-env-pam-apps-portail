...
class dataSideApp extends Vue {
  ...
  handleSelectedHardwareContextUpdate(newDynamicId) {
    ...
    const matchedContext = this.hardwareContextData.find(
      context => context.dynamicId === newDynamicId
    );

    // Set mainHardwareContext to the realId if a match is found
    this.mainHardwareContext = matchedContext ? matchedContext.nodes[0]?.realid : null;
  }
  ...
  async getHardwareContextByRelation(
    ...
    const matchedContext = this.hardwareContextData.find(
      context => context.dynamicId === this.selectedHardwareContext
    );
