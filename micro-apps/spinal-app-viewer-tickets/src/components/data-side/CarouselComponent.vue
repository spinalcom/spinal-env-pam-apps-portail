<template>
  <div v-if="image_list.length > 0" style="width: 100%;height: 100%;" class="d-flex flex-column">
    ...
    <div class="d-flex flex-row flex-grow-1 justify-space-between" style="
      ...
      <div class="d-flex align-center px-2">
        <v-btn style="color: white;width: 25px;height: 25px; background-color: #ffffff40" fab @click="
      ...
      <div class="d-flex align-center px-2">
        <v-btn style="color: white;width: 25px;height: 25px; background-color: #ffffff40" fab
          @click="c_index = (c_index + 1) % image_list.length">
          <v-icon large style="font-size: 25px!important;">mdi-chevron-right</v-icon>
        </v-btn>
      </div>
    </div>
    <div class="d-flex justify-center flex-shrink-1" style="background-color: rgba(0, 0, 0, 0.6)">
      <div v-for="(img, i) in image_list" :key="i" class="d-flex align-center justify-center rounded-circle ma-1"
        style="width: 18px; height: 18px" :style="{
          'background-color': `rgba(255,255,255,${c_index === i ? '0.3' : '0'
            })`,
        }">
        <v-btn fab style="background-color: white; width: 10px; height: 10px" @click="c_index = i"></v-btn>
      </div>
    </div>
    <div class="d-flex justify-end flex-shrink-1" style="">
      <div class="text-center">
        <a style="color: #14202c;font-size: 11px;" :href="image_list[c_index].src" target="_blank"
          ...
          <v-icon class="ml-1" style="color: #14202c;font-size: 15px;">
            mdi-tray-arrow-down
          </v-icon>
        </a>
      </div>
