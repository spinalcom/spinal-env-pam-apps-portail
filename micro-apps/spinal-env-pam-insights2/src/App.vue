...
<template>
  <v-app v-if="pageSate === PAGE_STATES.loaded" class="app">
    ...
    <div class="dataBody">
      <viewerApp v-if="!vueChart" class="viewerContainer" :class="{ active3D: isActive3D }"></viewerApp>
      <sc-line-card v-else class="viewerContainer" :class="{ active3D: isActive3D }" :title="title"
        :labels="labelDisplay" :datasets="chartData" :step="labels.length / 4" :tooltipCallbacks="{
      ...
      <InsightApp class="appContainer" :DActive="isActive3D" :ActiveData="isActive"
        ...
        :selectedTime="temporalitySelected" :data="displayedData" @clickOnDataView="onDataViewClicked"
        @buttonClicked="toggleActive" @buttonClicked3D="toggleActive3D" @chartView="switchView" @full3D="full3D()">
      </InsightApp>
    </div>
  </v-app>
