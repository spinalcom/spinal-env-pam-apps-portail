  ...
  <v-app id="application">
    ...
    <v-main style="position: absolute; margin-top: 3.5%; width: 100%" v-else>
      ...
      <v-row id="stat-section">
        <v-col>
          <sc-double-stat-card
              :first-value="energyConsumption('year')"
              :first-unit="'KW'"
              ...
              :second-value="heatingConsumption('year')"
              :second-unit="'KW'"
              :second-title="'EN CHAUFFAGE'"
  ...
  computed: {
    ...
    percentageCalculator() {
      ...
      if(this.energyConsumption('day')!==0){
        p[2] = (this.heatingConsumption('day') * 100 / this.energyConsumption('day')).toFixed(2);
      }
      if(this.energyConsumption('week')!==0){
        p[1] = (this.heatingConsumption('week') * 100 / this.energyConsumption('week')).toFixed(2);
      }
      if(this.energyConsumption('year')!==0){
        p[0] = (this.heatingConsumption('year') * 100 / this.energyConsumption('year')).toFixed(2);
      }
      return p;
    },
    lastPeriodCompare() {
