...
<script lang="ts">
import  { ControlEndpoint } from '../models/ConfigInterface'

export default {
    name: 'SelectCategory',
    props: {
        controls_point: {
            type: Array as () => ControlEndpoint[],
            required: true
        },
        control_value: {
            type: Object as () => ControlEndpoint,
            required: false
        }
    ...
    computed: {
        computedControlValue: {
            ...
            set(newValue: ControlEndpoint) {
                this.change_endpoint(newValue);
            }
        }
    },
    mounted() {
        console.log('mounted', this.localControlValue)
  ...
  beforeDestroy() {
    ...
    data() {
        return {
                localControlValue: this.control_value as ControlEndpoint,
                showIn: false,
                icon: 'mdi-chevron-down'
        }
    },
    watch: {
        control_value(newValue: ControlEndpoint) {
            this.localControlValue = newValue;
        }
    },
