    ...
    <v-dialog class="dialog-content" v-model="show" persistent width="65%"
        ...
        <div class="content-form">
            <form style="width: 100%; height: 100%;" @submit.prevent="validateChanges">
                <div class="header">
                    <h2>Modification de la catégory d'attribut</h2>
                    <div>
                        <button type="submit" class="save-btn">Enregistrer</button>
                        <button type="reset" class="cancel-btn" @click="closeDialog">Fermer</button>
                    </div>
                </div>
                <h2 class="ml-2 mb-2 mt-2">Nom de la catégory :</h2>
                <v-text-field v-model="localCopy.name" label="nom de la catégory" outlined dense hide-details
                    class="mx-4 mb-2 mt-2" style="width: 95%;"></v-text-field>
            </form>
...
export default {
    ...
    methods: {
        ...
        validateChanges() {
            console.log('Copie validée :', this.localCopy ,this.item);
            this.$emit('validatedcate', this.id, this.item?.dynamicId , this.localCopy ); 
            this.closeDialog(); 
        }
    }
}
