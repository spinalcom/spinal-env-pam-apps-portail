<template>
  <v-row style="display: flex; flex-wrap: nowrap; align-items: center; justify-content: flex-end; gap: 10px; padding: 10px; width: max-content;">
    <v-icon style="cursor: pointer;" ref="btn" @click="changeOverflowItemMenu(item.dynamicId)">mdi-dots-vertical</v-icon>
    <div class="over-menu" ref="over_content" v-if="show">
        <ul class="over-menu__content" style="width: 100%; padding-top: 10px; padding-left: 0px; display: flex; flex-direction: column; gap: 10px; ">
          <li class="list-menu" @click="showDoc(item.dynamicId, item.Name)">
                    <v-icon style="width: max-content; height: max-content; background-color: rgba(203 213 225 0.5); border-radius: 50%; font-size: 15px; color: #14202c; cursor: pointer;"  >
                      mdi-eye
                    </v-icon>
                    <span>Voir le document</span>
                  </li>
                  <li class="list-menu" @click="downloadFile(item.dynamicId)">
