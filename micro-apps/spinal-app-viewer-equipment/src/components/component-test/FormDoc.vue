      ...
      <form style="width: 100%; height: 100%" @submit.prevent="uploadDoc">
        <div class="header">
          ...
          <div>
            ...
            <button type="reset" class="cancel-btn" @click="closeDialog"
              >Fermer</button
            >
          </div>
        </div>
        <div class="content-input">
          ...
          <div class="file-content">
            <div
              ...
              :class="{ 'animation-remove-file': remove_animation == idx }"
            >
              <div style="width: 100%; display: flex; gap: 10px">
                <v-icon :style="{ color: iconFile(item.name).color }">{{
                  iconFile(item.name).name
                }}</v-icon>
                <span>{{ item.name }}</span>
              </div>
              <div>
  ...
  methods: {
    ...
    async uploadDoc() {
      ...
      if (filesData.length != 0) {
        ...
        const res = await this.$store.dispatch(ActionTypes.ADD_DOC, {
          buildingId,
    ...
    pushFile() {
      ...
      for (let index = 0; index < files.length; index++) {
        const type_file = files_types.find((type) =>
          files[index].name.includes(type)
        );
        if (type_file != undefined) {
          ...
          this.valid_message =
            'Les fichiers ' +
            this.getExtension(files[index].name)[0] +
