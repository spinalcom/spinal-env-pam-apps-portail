    ...
    <div class="top-bar">
      <div style="width: 100%; text-align: center">
        <span class="name-file" v-if="fileName">
          <v-icon :style="{ color: getIcon(fileName_with_ext).color }">{{
            getIcon(fileName_with_ext).name
          }}</v-icon>
          {{ this.fileName }}</span
        >
      </div>
      <div>
        <v-btn text class="btn-closed" @click="closeDialog"> Fermer </v-btn>
      </div>
    </div>
    <div
      v-if="loader"
...
export default {
  ...
  watch: {
    referenceId: async function (val) {
      ...
      const file = [
        this.$store.dispatch(ActionTypes.GET_FILE, {
    ...
    showFile(type: string, url: string) {
      this.show.map(async (item, index) => {
        if (type === 'pdf') {
          ...
          console.log('DATA :',this.tableExcelData);
