<template>
  ...
  <template v-slot:[`item.sum`]="{ item }">
    <span class="text">{{ item.sum.toFixed(2) }} L</span>
  </template>
   <template v-slot:[`item.squareMeter`]="{ item }">
    <span class="text">{{ item.squareMeter.toFixed(2) }} L</span>
  </template>
...
export default {
  ...
  props: ['context', 'temporality'],
  data: () => ({
    headers: [
      { text: 'Nom', value: 'name' },
      { text: 'Surface', value: 'surface', align: 'center' },
      { text: 'Consommation', value: 'sum', align: 'center'},
  ...
  methods: {
    dynamicHeaders() {
      ...
      return this.headers.map(header => {
        if(header.text == 'Consommation')
        header.text = header.text + ' ' + dynamicText;
        else if (header.text == 'square')
        header.text = 'Consommation' + ' ' + dynamicText + ' au m²';
        return header;
      });
    }
  },
