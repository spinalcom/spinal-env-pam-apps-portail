<template>
    <v-card class="table-container spinal-scrollbar">
        ...
        <v-simple-table>
            ...
            <tbody>
                <tr v-for="(ticket, index) in sortedData" :key="index" @click="handleClickOfLocate(ticket)"
                    :class="{ 'selectedTicket-class': (selectedTicket && selectedTicket.dynamicId == ticket.dynamicId) || ticket.isSelected }">
                    ...
                    <td style="width: 13%;">
                        <div class="d-flex flex-row row-style">
                            ...
                            <div
                                :style="{ fontWeight: ticket.step.name === 'Attente de réalisation' ? 'bold' : 'normal' }">
...
class TicketTable extends Vue {
    ...
    moveTicketsToTop(ticketsToMove: Array<any>) {
        this.resetTickets();
        console.log("Tickets to move: ", ticketsToMove);
    ...
    mounted() {
        ...
        // console.log("Mounted from Datatable ", this.data);
    }
    beforeDestroy() {
...
.selectedTicket-class {
    /* background-color: #14202c20 !important; */
