...
async function FctViewerIteract(
  ...
  { state, dispatch, commit }: AugmentedActionContextAppData,
  payload: any
): Promise<any> {
  ...
  const data: IViewInfoItemRes[] = await dispatch(ActionTypes.GET_VIEWER_INFO, {
    item: payload,
  });
...
export const actions: ActionTree<StateAppData, StateAppData> & Actions = {
  ...
  async [ActionTypes.GET_GEOGRAPHIC_ITEMS_GROUPS](
    { commit, state }: AugmentedActionContextAppData,
    { forceUpdate, buildingId }
  ): Promise<{ [key: string]: INodeItem }> {
    const spinalAPI = SpinalAPI.getInstance();

    if (
      typeof ApiIteratorStore[ActionTypes.GET_GEOGRAPHIC_ITEMS_GROUPS] ===
      "undefined"
    ) {
    ...
    if (
      ...
      ApiIteratorStore[ActionTypes.GET_GEOGRAPHIC_ITEMS_GROUPS][buildingId] =
        spinalAPI.createIteratorCall(getGeographicItemsGroups, buildingId);
    }

    const items = await ApiIteratorStore[
      ActionTypes.GET_GEOGRAPHIC_ITEMS_GROUPS
    ][buildingId].next();
    return items?.value;
  },
  ...
  async [ActionTypes.OPEN_VIEWER](
    ...
    playload: { onlyThisModel: boolean; item: any }
  ): Promise<void> {
