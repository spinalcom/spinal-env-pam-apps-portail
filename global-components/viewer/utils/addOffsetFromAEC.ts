...
export async function addOffsetFromAEC(aecPath: string, viewer: Autodesk.Viewing.Viewer3D, buildingId: string): Promise<THREE.Vector3> {
  ...
  const path = getAPINormalisePath(aecPath, buildingId)
  const aecModelData = await getAecModelData(path);  // recuperer l'aecData de la première maquette de la scene 1 (scene par default)
  if (aecModelData) {
